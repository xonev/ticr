...
* [`src/ti_dice_roll/styles.cljs`](src/ti_dice_roll/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/ti_dice_roll/`](src/ti_dice_roll/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/ti_dice_roll/core.cljs): contains the SPA entry point, `init`
* [`test/ti_dice_roll/`](test/ti_dice_roll/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `ti-dice-roll` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/ti_dice_roll/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns ti-dice-roll.example
  (:require [ti-dice-roll.config :as config])
